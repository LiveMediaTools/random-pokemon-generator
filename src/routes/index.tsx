  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 pb-8 pt-10 md:px-6 md:pb-10 md:pt-12 xl:pb-16 xl:pt-20">
          <motion.div
          ...
          >
            ...
            <h1 className="mt-3 font-display text-3xl font-extrabold leading-[1.05] tracking-tight text-foreground md:text-5xl xl:text-6xl">
              Random Pokemon Generator
            </h1>
            <p className="mt-3 text-sm text-muted-foreground md:text-base xl:text-lg">
          ...
          <div className="mt-6 grid gap-3 lg:grid-cols-4">
            {quickRoutes.map((route) => (
              <Link
                key={route.to}
                to={route.to}
                className="rounded-2xl border border-border/70 bg-background/70 p-3.5 text-left shadow-[var(--shadow-card)] backdrop-blur-sm transition-all hover:-translate-y-0.5 hover:border-primary md:p-4"
              >
                <div className="font-display text-sm font-bold md:text-base">{route.title}</div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 md:px-6 lg:py-14">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {featuredPokemon.map((pokemon) => (
            <article key={pokemon.id} className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)] md:p-5">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {PRESETS.slice(0, 8).map((p) => (
            <Link
              ...
              to="/challenges/$slug"
              params={{ slug: p.slug }}
              className="group rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)] transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-pop)] md:p-5"
            >
      ...
      <section className="border-y border-border bg-surface">
        <div className="mx-auto grid max-w-7xl gap-4 px-4 py-10 md:px-6 lg:grid-cols-3 lg:gap-6 lg:py-14">
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 md:px-6 lg:py-14">
        ...
        <SectionHead className="mt-10 lg:mt-12" title="Browse by generation" subtitle="From Kanto to Paldea — every regional dex with its own random generator." />
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-6 lg:py-14">
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3 lg:gap-6">
            {[
              ...
              <div key={n} className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)] md:p-5 lg:p-6">
                <div className="font-display text-2xl font-extrabold text-primary md:text-3xl">{n}</div>
                <div className="mt-2 font-display text-base font-bold md:text-lg">{t}</div>
      ...
      {/* FAQ */}
      <section className="mx-auto max-w-3xl px-4 py-10 md:px-6 lg:py-14">
        <SectionHead title="Frequently asked questions" subtitle="" />
        <div className="mt-6 space-y-2">
          {faqs.map((f) => (
            <details key={f.q} className="group rounded-2xl border border-border bg-card p-4 md:p-5">
              <summary className="cursor-pointer list-none font-display text-sm font-semibold md:text-base">
...
function SectionHead({ title, subtitle, className = "" }: { title: string; subtitle?: string; className?: string }) {
  return (
    <div className={className}>
      <h2 className="font-display text-xl font-extrabold tracking-tight md:text-2xl xl:text-3xl">{title}</h2>
...
function Why({ icon: Icon, title, desc }: { icon: any; title: string; desc: string }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)] md:p-5 lg:p-6">
      <div className="grid h-9 w-9 place-items-center rounded-xl bg-[var(--gradient-primary)] text-zinc-950 shadow-[var(--shadow-pop)] md:h-10 md:w-10">
        <Icon className="h-5 w-5" />
      </div>
      <h3 className="mt-3 font-display text-base font-bold md:text-lg">{title}</h3>
